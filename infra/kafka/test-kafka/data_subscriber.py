...
def create_table(db_connect):
    create_table_query = """
    CREATE TABLE IF NOT EXISTS mnist_prediction (
        id SERIAL PRIMARY KEY,
        timestamp timestamp,
        minist_accuracy float
    );"""
...
def insert_data(db_connect, data):
    insert_row_query = f"""
    INSERT INTO mnist_prediction
        (timestamp, minist_accuracy)
        VALUES (
            '{data["timestamp"]}',
            {data["minist_accuracy"]}
        );"""
...
def subscribe_data(db_connect, consumer):
    for msg in consumer:
        ...
        response = requests.post(
            url="http://api-with-model:8000/accuracypredict",
